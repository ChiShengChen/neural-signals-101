...
# ## ✅ 觀念檢查
#
# 1. 一個 epoch 陣列的形狀是 `(n_trials, n_channels, n_times)`。要得到「**每個 channel、
#    每個 trial 一個數字**」,你該對哪一軸取平均?
# 2. 為什麼 `X` 和 `y` 必須在第 0 軸對齊?如果你重排了 `X` 的 trial 卻沒重排 `y` 會怎樣?
# 3. 這裡有兩個資料集的取樣率不同(例如 160 vs 250 Hz)。為什麼為其中一個寫的濾波器或 FFT
#    用在另一個上會出錯?
#
# **解答:**(1) `axis=2`(時間)→ `(n_trials, n_channels)`。(2) `y[i]` 是 `X[i]` 的標籤;
# 只重排其一會讓每個 trial 被悄悄貼錯標籤。(3) 兩者都以「取樣率的比例」為單位,所以同一個
# 以 Hz 表示的截止頻率,在不同 `sfreq` 下對應到不同的正規化頻率——永遠把 `sfreq` 跟著資料帶走。
