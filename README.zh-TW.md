...
> 視覺化呈現(不證明),卡住時還有[詞彙表](docs/GLOSSARY.md)和
...
> 📚 第一次來?完整的學習設計與理念見 [`docs/CURRICULUM.md`](docs/CURRICULUM.md)。
...
你就能認出紅色長條偷溜進真實專案的**六種不同方式**(以及各自怎麼避免)。
...
> [`docs/GETTING_STARTED.md`](docs/GETTING_STARTED.md)——包含一鍵**在 Colab 執行**(免
> 安裝)的路徑,以及 5 題 Python 自我檢測。
> 建議隨時開著的分頁:[詞彙表](docs/GLOSSARY.md) ·
> [FAQ](docs/FAQ.md)(「我的分數算好嗎?」)· [速查表](docs/CHEATSHEET.md) ·
> [疑難排解](docs/TROUBLESHOOTING.md)。
...
| **熟 ML、不熟訊號** | 略讀 **02**、做 **03**;聚焦 **01、04–07、10、12、13**。 |
| **熟神經、不熟 ML** | 略讀 **01**;做 **02、03、11**;聚焦 **04–09、12**。 |

**所有人都該讀第 12 章(評估陷阱)與第 13 章(倫理與反炒作)**——它們是整份教學的重點。
...
|---|---|---|---|---|
| 00 | [環境設定與資料](notebooks/00_setup_and_data.ipynb) | 生態系、檔案格式、載入與繪圖、**陣列形狀心智模型** | ★ | ~3–5 分 |
| 01 | [神經訊號是什麼](notebooks/01_what_are_neural_signals.ipynb) | EEG 物理來源、volume conduction、10-20 系統、雜訊在哪 | ★★ | ~1 分 |
| 02 | [**從零開始的 ML**](notebooks/02_ml_from_zero.ipynb) 🆕 | 過擬合、train/val/test、*為何 test 神聖不可碰*(玩具資料,無 EEG) | ★★ | ~1 分 |
| 03 | [**看得見的數學**](notebooks/03_math_you_can_see.ipynb) 🆕 | Fourier、共變異、特徵向量/CSP 幾何——視覺化,不證明 | ★★★ | ~1 分 |
| 04 | [DSP 基礎](notebooks/04_dsp_basics.ipynb) | 取樣、aliasing、量化、濾波器、notch、re-reference | ★★★ | ~1 分 |
| 05 | [前處理與去雜訊](notebooks/05_preprocessing_and_denoising.ipynb) | 假影、ICA、ASR 式清理、epoching、baseline | ★★★ | ~1–2 分 |
| 06 | [頻域](notebooks/06_frequency_domain.ipynb) | FFT、Welch PSD、STFT、wavelet、band power、時頻權衡 | ★★★ | ~1 分 |
| 07 | [特徵工程](notebooks/07_feature_engineering.ipynb) | **接回生理**的特徵(ERD/ERS)、CSP、Riemannian 共變異 | ★★★★ | ~1–2 分 |
| 08 | [古典 ML](notebooks/08_classical_ml.ipynb) | LDA/SVM/RF/Riemann 用 Pipeline、**正確的交叉驗證** | ★★★ | ~2–3 分 |
| 09 | [深度學習](notebooks/09_deep_learning.ipynb) | EEGNet、ShallowConvNet、DeepConvNet、LSTM、小型 Transformer | ★★★★ | ~3–5 分 |
| 10 | [範式與應用](notebooks/10_paradigms_and_applications.ipynb) | MI、P300/ERP、SSVEP、睡眠、癲癇、brain-to-text——**為何有效** | ★★★ | ~2–3 分 |
| 11 | [**統計直覺**](notebooks/11_statistics_intuition.ipynb) 🆕 | 抽樣變異、mean±std、chance≠1/k、「小差距其實是雜訊」 | ★★★ | ~1–2 分 |
| 12 | [**評估與陷阱**](notebooks/12_evaluation_and_pitfalls.ipynb) ⭐ | 六組 WRONG→RIGHT 對照;最重要的一章 | ★★★★ | ~2–4 分 |
| 13 | [**神經倫理與反炒作**](notebooks/13_neuroethics_and_anti_hype.ipynb) 🆕 | 隱私、consent、neuro-rights、「offline 95% ≠ 能用的 BCI」、炒作=洩漏的孿生 | ★★ | ~1 分 |
| 14 | [Capstone](notebooks/14_capstone.ipynb) | 原始資料 → 誠實報告,對上**隱藏 held-out 排行榜** | ★★★★ | ~2–4 分 |
| 15 | [即時與硬體](notebooks/15_realtime_and_hardware.ipynb) 🆕 | 模擬串流推論 + 低成本硬體路徑(OpenBCI/Muse) | ★★ | ~1–2 分 |

\*首次執行會下載並快取資料;之後快很多。每章都有前置 + 難度框、「先猜再跑」cell、以及
✅ 觀念檢查。附解答的額外習題:[`docs/SOLUTIONS.md`](docs/SOLUTIONS.md)。
...
notebooks/      00_setup … 15_realtime  (.ipynb,由 notebooks/_src/*.py 產生)
...
新增章節或功能請見 [CONTRIBUTING.md](CONTRIBUTING.md),一頁 API + 評估檢查表見
